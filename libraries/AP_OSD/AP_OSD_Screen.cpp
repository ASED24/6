...
const AP_Param::GroupInfo AP_OSD_Screen::var_info[] = {
    ...
    // @Description: Displays most active esc's temp
    // @Values: 0:Disabled,1:Enabled
    ...
    // @Description: Displays most active esc's rpm
    // @Values: 0:Disabled,1:Enabled
...
const AP_Param::GroupInfo AP_OSD_Screen::var_info2[] = {
    ...
    // @Param: ESC_IDX
    // @DisplayName: ESC_IDX
    // @Description: Index of the ESC to use for displaying ESC information. 0 means use the most active ESC.
    AP_SUBGROUPINFO(esc_index, "ESC_IDX", 10, AP_OSD_Screen, AP_OSD_Setting),
#endif
...
void AP_OSD_Screen::draw_esc_temp(uint8_t x, uint8_t y)
{
    ...
    if (esc_index > 0) {
        if (!AP::esc_telem().get_motor_temperature(esc_index, etemp)) {
            return;
        }
    }
...
void AP_OSD_Screen::draw_esc_rpm(uint8_t x, uint8_t y)
{
    ...
    if (esc_index > 0) {
        if (!AP::esc_telem().get_rpm(esc_index, rpm)) {
            return;
        }
...
void AP_OSD_Screen::draw_esc_amps(uint8_t x, uint8_t y)
{
    ...
    uint8_t esc = AP::esc_telem().get_active_esc();
    if (esc_index > 0) {
        if (!AP::esc_telem().get_current(esc_index, amps)) {
            return;
        }
